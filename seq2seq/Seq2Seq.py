...
class RNN_en(nn.Module):
    ...
    def forward(self, input_sentence):
        # (seq_length,1,dim_embedding)
        embedded = self.embedding(input_sentence.view(-1,1))
        hidden = torch.zeros(
            (self.num_gru_layer, 1, self.hidden_size), device=device)
        for x in embedded:
...
class RNN_de(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        out, hidden = self.rnn(embedded.view(1, 1, -1), 
                                hidden.view(self.num_gru_layer, 1, self.hidden_size))
...
if __name__ == "__main__":
    ...
    print(output)
    input, output = tensorFromPair(rand, input_lang, output_lang)
    seq2seq = Seq2Seq(input_lang, output_lang,
                      nn.NLLLoss(), 1024, 2).to(device)
